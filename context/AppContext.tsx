   ...
   useEffect(() => {
      const hydrate = async () => {
         ...
         try {
            ...
            // ✅ SIEMPRE obtener precios frescos desde el backend (cache solo como fallback)
            try {
               let preciosFrescos: any[] = [];

               if (userObj?.pais_id) {
                  // console.log("\x1b[33m[AppContext] 💰 Obteniendo precios frescos para país:", userObj.pais_id);
                  preciosFrescos = await fetchPrices(userObj.pais_id);
               } else {
                  const locale = Localization.getLocales()[0];
                  const regionCode = locale?.regionCode ?? "MX";
                  // console.log("\x1b[36m[AppContext] 🌐 Locale del dispositivo:", regionCode);
                  const paisPorLocale = paises.find((p) => p.codigo_pais?.toUpperCase() === regionCode.toUpperCase());
                  const paisId = paisPorLocale?.id ?? 1;
                  // console.log(
                  //    "\x1b[33m[AppContext] 💰 Obteniendo precios frescos para locale:",
                  //    regionCode,
                  //    "id:",
                  //    paisId,
                  // );
                  preciosFrescos = await fetchPrices(paisId);
               }

               if (preciosFrescos.length > 0) {
                  precios = preciosFrescos;
                  // console.log("\x1b[32m[AppContext] ✅ Precios frescos obtenidos:", precios.length);
               }
            } catch (e) {
               // console.log("\x1b[33m[AppContext] ⚠️ Error obteniendo precios frescos, usando cache:", e);
               // precios ya tiene el valor del cache si existía
            }
